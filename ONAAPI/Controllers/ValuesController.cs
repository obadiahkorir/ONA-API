...
namespace ONAAPI.Controllers
{
    ...
    public class ValuesController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/values/MigrateOnaData/{project_number}/{form_number}")]
        public async Task MigrateOnaData(string project_number, string form_number)
        {
            try
            {

                HttpResponseMessage httpResponseMessage = new HttpResponseMessage();
                string FormNumber = form_number;
                string ProjectNumber = project_number;
                if (!string.IsNullOrEmpty(FormNumber))
                {
                    ...
                    if (bbConnected)
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            var SharepointSubSite = SharepointURL + ProjectNumber;
